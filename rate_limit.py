...
# In-memory data structure to track login attempts
login_attempts = {}
BLOCK_TIME = timedelta(minutes=15)  # Block for 15 minutes after 3 failed attempts
MAX_ATTEMPTS = 3
...
def login():
    if request.method == 'POST':
        ...
        if username in login_attempts and login_attempts[username]['count'] >= MAX_ATTEMPTS:
            if datetime.now() < login_attempts[username]['block_until']:
                print(Fore.GREEN + 'Too many failed attempts. Please try again later.', 'error'+Style.RESET_ALL)
                return render_template('login.html')
        ...
        if username not in login_attempts:
            login_attempts[username] = {'count': 1, 'block_until': datetime.now()}
            print(Fore.GREEN + login_attempts+ Style.RESET_ALL)
        else:
            login_attempts[username]['count'] += 1
            if login_attempts[username]['count'] >= MAX_ATTEMPTS:
                login_attempts[username]['block_until'] = datetime.now() + BLOCK_TIME
                print(Fore.GREEN +login_attempts+ Style.RESET_ALL)
...
def api_login():
    ...
    if username in login_attempts and login_attempts[username]['count'] >= MAX_ATTEMPTS:
        if datetime.now() < login_attempts[username]['block_until']:
            return jsonify({"msg": "Too many failed attempts. Please try again later."}), 429
